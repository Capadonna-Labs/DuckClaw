...
export default function ProjectsPage() {
  ...
  const reload = useCallback(() => {
    async function loadAll() {
      const [legacyProjects, dbProjects, templates] = await Promise.all([
        adminService.listForgeProjects(),
        adminService.listWorkspaceProjects(),
        adminService.listTemplates(),
      ]);
      setItems(legacyProjects);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.18em] text-gov-gray-400">
            Legacy filesystem
          </p>
          <h2 className="text-xl font-black dark:text-dark-text">Proyectos en disco</h2>
        </div>
        {items.length === 0 ? (
          <EmptyState
            variant="empty"
            customMessage="Sin proyectos legacy. Crea proyectos DB-first arriba o define presets en .env."
