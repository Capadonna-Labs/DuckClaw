...
3. **`packages/agents/train/scripts/serve/start_mlx.sh`** arranca **`serve/run_mlx_lm_server.py`** (delega en `mlx_lm server` de Apple) y pasa `--adapter-path` cuando `MLX_ADAPTER_PATH` existe; incluye una capa fina de **reparación de JSON** en tool calls Gemma 4 cuando la salida del modelo no es JSON estricto. Rutas relativas se resuelven contra la raíz del monorepo.
