...
| `[SYS]` en subgrafo del worker | Worker asignado (`research_worker`, `default`, …) | Tras delegación, línea breve `Delegación: manager -> <id>`. |
...
El campo HTTP **`worker_id`** de la ruta (p. ej. `/api/v1/agent/research_worker/chat`) **no** tiene por qué coincidir con el worker de observabilidad en `[REQ]`/`[PLAN]`.
...
- **`run_name`** (columna **Name** en LangSmith): el **worker/template** — p. ej. `Manager` (orquestador), `default`, `research_worker`. El tenant no va en el nombre; sí en tags/metadata.
