...
- Mantener `workers/factory.py` como superficie de alto riesgo: tras extraer discovery/listado, retry visual, truncado de tool outputs, provider input budget y context monitor, los siguientes cortes deben migrar tools/capabilities y runtime assembly hacia owners pequenos DB-first o extensiones, no a nuevas ramas por dominio.
