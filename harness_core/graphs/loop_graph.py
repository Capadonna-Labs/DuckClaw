"""LangGraph loop thermostat: telemetry → distance → plan → dispatch."""
...
from harness_core.states.loop_state import CorrectiveAction, HomeostasisTarget, LoopState
...
def _targets_from_state(state: LoopState) -> HomeostasisTarget:
...
def _invoke_plan_llm(
    ...
    *,
    ...
    sys = SystemMessage(
        content=(
            "You are the loop infrastructure planner. Respond ONLY with a JSON array of corrective actions. "
...
def node_sweep_telemetry(state: LoopState) -> LoopState:
    out: LoopState = {**state, "status": "sweeping"}
...
def node_calculate_distance(state: LoopState) -> LoopState:
    out: LoopState = {**state, "status": "calculating"}
...
def node_plan_correction(state: LoopState, *, llm: Any | None = None) -> LoopState:
    out: LoopState = {**state, "status": "planning"}
    if state.get("status") == "failed":
        return out
...
def node_dispatch_actions(state: LoopState) -> LoopState:
    out: LoopState = {**state, "status": "dispatching"}
    if state.get("status") == "failed":
    ...
    for raw in planned:
        ...
        if action.requires_hitl and action.action_type in (
            ...
            notify_admin_alert(
                admin_chat,
                f"loop HITL: {action.action_type} — {action.reason}",
                worker_id=worker_id,
                tenant_id=tenant_id,
                distance_vector=distance,
                actions=[entry],
...
def build_loop_graph(*, llm: Any | None = None) -> StateGraph:
    """Build loop graph; optional llm enables structured plan_correction."""
    builder = StateGraph(LoopState)
    builder.add_node("sweep_telemetry", node_sweep_telemetry)
    builder.add_node("calculate_distance", node_calculate_distance)

    if llm is not None:
        def _plan(state: LoopState) -> LoopState:
...
def invoke_loop_run(initial: LoopState, *, llm: Any | None = None) -> LoopState:
    """Compile and invoke loop graph with run_id default."""
    state: LoopState = {
        "run_id": str(uuid.uuid4()),
        "status": "pending",
        **initial,
    }
    graph = build_loop_graph(llm=llm).compile()
...
loop_graph = build_loop_graph().compile()
build_meditate_graph = build_loop_graph
invoke_meditate_run = invoke_loop_run
meditate_graph = loop_graph
