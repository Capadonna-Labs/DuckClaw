...
export function PlataformaNavSelector({ icon: Icon, label, onNavigate }: Props) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const activeTab = parsePlataformaTab(searchParams.get('tab'));
  const hubActive = isPlataformaPath(pathname);
    ...
    <div>
      ...
      {open && (
        <div className="ml-7 mt-1 space-y-0.5 border-l border-white/10 pl-3">
          {PLATAFORMA_TABS.map((tab) => {
            const childActive = hubActive && activeTab === tab.id;
            return (
              <Link
                key={tab.id}
                href={plataformaTabHref(tab.id)}
                onClick={() => onNavigate?.()}
                className={cn(
