"""HITL validation for /loop homeostasis."""
...
from duckclaw.commands.hitl import execute_loop_approve, execute_loop_reject
from duckclaw.commands.loop import build_meditate_self_system_event_message
...
def test_approve_without_pending_errors() -> None:
    ...
    out = execute_loop_approve(db, "c4", "")
...
def test_reject_validation_with_rationale() -> None:
    ...
    out = execute_loop_reject(db, "c5", f"{vid} otra vez")
    assert "no hay validación" in out.lower()
...
def test_fly_meditate_approve_success() -> None:
    ...
    set_chat_state(db, "c6", "loop_active", "1")
    set_chat_state(db, "c6", "loop_delta_seconds", "900")
    created = create_pending_validation(db, "c6", tenant_id="t1", snapshot={})
    vid = created["validation_id"]
    out = execute_loop_approve(db, "c6", vid, tenant_id="t1")
    assert "homeostasis confirmada" in out.lower() or "modo `/loop` detenido" in out.lower()
    assert vid in out
    assert get_chat_state(db, "c6", "loop_active") == "0"
    assert get_chat_state(db, "c6", "loop_delta_seconds") == "0"
