...
- **Heartbeat Telegram:** si `sandbox_heartbeat` es true y existe `DUCKCLAW_HEARTBEAT_WEBHOOK_URL`, antes de `run_sandbox` se notifica al usuario; desactivar con `DUCKCLAW_SANDBOX_HEARTBEAT=false`.