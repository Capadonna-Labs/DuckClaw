...
| `IBKR_EXECUTE_ORDER_URL` | POST JSON mínimo `{"signal_id","paper":true}` (opcional; sin URL la orden no se envía al broker tras HITL). El gateway puede **enriquecer** el cuerpo con `ticker`, `signal_type`, `proposed_weight`, `mandate_id` (Quant Trader) o `action`, `quantity` (Finanz) para que el VPS no dependa de copiar el `.duckdb`; el receptor inyecta `DUCKCLAW_EMBEDDED_EXECUTE_JSON` en el hook. Cabecera **`X-Duckclaw-IBKR-Account-Mode: paper|live`** (coherente con sesión/env; Finanz siempre paper). Modo efectivo en el receptor: `OHLCV_BROKER_EXECUTE_FORCE_PAPER`, luego cabecera, luego JSON (default paper). **`POST /api/broker/execute`** en `services/ibkr-ohlcv-api`. TIF/brackets: `OHLCV_EXECUTE_ORDER_*` o tu servicio en el VPS. |
