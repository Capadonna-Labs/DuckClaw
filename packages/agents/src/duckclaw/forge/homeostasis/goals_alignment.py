"""Evaluación de alineación entre objetivos /crons y contexto observable."""
...
_NUDGE_OPENERS: tuple[str, ...] = (
    ...
    "Revisión proactiva: comparo portfolio/sesión con tus /crons antes de proponerte pasos.",
...
def refresh_goal_observations(db: Any, chat_id: Any, worker_id: str) -> list[dict]:
    """
    Actualiza observed_value en goals sin LLM (PnL en chat_state, etc.).
    Devuelve la lista de goals (mutada en memoria y persistida si hubo cambios).
    """
    from duckclaw.graphs.on_the_fly_commands import get_chat_state, get_manager_goals, set_manager_goals
    ...
    for g in goals:
        ...
        if pnl_val is not None and (
            ...
            if dd is not None:
                new_obs = dd
        elif g.get("observed_value") is not None:
...
def _read_quant_drawdown_pct(db: Any) -> float | None:
    """Lee drawdown de sesión activa si existe en quant_core.trading_sessions."""
    try:
        raw = db.query(
            "SELECT session_goal, anchor_equity FROM quant_core.trading_sessions "
...
def build_alignment_nudge_system_event(
    ...
    *,
    ...
    if obj == "overnight_gap_squeeze":
        extra = " Contexto Quant: prioriza prep/MOC y gap squeeze según sesión activa."
    elif obj == "rebalance_hrp":
        extra = " Contexto Quant: prioriza desviación cartera vs pesos HRP."
    directive = (
        ...
        "Si falta observed en el informe, invoca `assess_crons_alignment` o herramientas de datos (portfolio, CFD, read_sql). "
