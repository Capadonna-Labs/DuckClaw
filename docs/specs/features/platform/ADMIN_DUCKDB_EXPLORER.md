...
| `GET` | `/duckdb/legacy-schemas` | Lista schemas no canónicos y tablas legacy en `main` candidatas a limpieza; no oculta datos. |
| `POST` | `/duckdb/legacy-schemas/drop` | Body `{ "schemas": ["..."], "main_tables": ["..."], "vault_path?", "confirm": "DROP_LEGACY_SCHEMAS" }`; elimina con confirmación explícita y allow-list. |
...
- La limpieza de schemas o tablas `main` ajenas al perfil se hace por endpoint dedicado, con modal de confirmación, auditoría y allow-list de nombres candidatos. No se ejecuta DDL arbitrario desde el editor SQL.
- La allow-list default incluye schemas legacy conocidos y tablas `main.leila_orders` / `main.leila_products`; puede extenderse con Runtime Settings DB-first (`duckdb.legacy_schemas`, `duckdb.legacy_main_tables`) y fallback `.env` (`DUCKCLAW_ADMIN_DUCKDB_LEGACY_SCHEMAS`, `DUCKCLAW_ADMIN_DUCKDB_LEGACY_MAIN_TABLES`).
