...
APP_FACTORY = Path("services/api-gateway/gateway_app_factory.py")
...
def test_gateway_lifespan_has_no_redis_config_set_or_docker() -> None:
    ...
    # Shutdown closes the redis client owned by lifespan (not app.state.redis).
    lifespan_end = source.index("await _redis_client.aclose()")
