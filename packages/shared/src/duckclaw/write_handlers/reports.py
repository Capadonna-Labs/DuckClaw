...
def _require_template(conn: Any, template_id: str, tenant_id: str) -> dict[str, Any]:
    row = conn.execute(
        """
        SELECT template_id, tenant_id, owner_email, section_schema_json, template_uri, active
...
def _apply_upsert_report_template(conn: Any, payload: dict) -> None:
    ...
    existing = conn.execute(
        "SELECT template_id FROM main.admin_report_templates WHERE template_id = ?",
        [template_id],
    ).fetchone()
    ...
    if existing:
        conn.execute(
            ...
            SET tenant_id = ?, owner_email = ?, name = ?, description = ?, template_uri = ?,
                section_schema_json = ?, analyzer_mode = ?, visibility = ?, active = true,
                updated_at = CURRENT_TIMESTAMP
            WHERE template_id = ?
            """,
            [tenant_id, owner, name, description, template_uri, schema_json, analyzer_mode, visibility, template_id],
        )
    else:
        conn.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [template_id, tenant_id, owner, name, description, template_uri, schema_json, analyzer_mode, visibility],
        )
