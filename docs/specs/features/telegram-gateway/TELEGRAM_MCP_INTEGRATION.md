...
3. **Ejecución (Egress):** Cuando un Worker (ej. `support` o `bi_analyst`) decide responder, LangGraph intercepta la salida y llama a la tool expuesta por el servidor MCP (`telegram_send_message` o `telegram_send_photo`).
