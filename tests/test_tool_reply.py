...
from duckclaw.utils.tool_reply import looks_like_tabular_account_rows_json
...
def test_looks_like_tabular_account_rows_json() -> None:
    rows = [
        {
            "id": "1",
            "name": "Bancolombia",
    ...
    assert looks_like_tabular_account_rows_json(json.dumps(rows))
    assert not looks_like_tabular_account_rows_json('{"a": 1}')
