...
- `duckclaw.workers.skill_tool_registry` es el owner de registro lazy de tools por `skill_configs`. `workers/factory.py` ya no importa bridges concretos (`reddit_bridge`, `google_trends_bridge`, `openweather_bridge`, `comfyui_bridge`, etc.) ni contiene ramas directas por config vendor para registrar herramientas; solo invoca las fases genericas pre/post LLM del registry.
